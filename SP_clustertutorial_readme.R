...
library(ape)
setwd("/Users/taylorsehein/github/WHOI_repo/SP2018")
...
#split taxonomy into levels, preserving 'taxonomy' column too
colnames(tax_key)[2] <- 'taxonomy'
tax_key$taxonomy_copy <- tax_key$taxonomy
tax_key <- tax_key %>%
  separate(taxonomy_copy, into = c('Level1', 'Level2',
           'Level3', 'Level4', 'Level5', 'Level6',
           'Level7'), sep = ';')
tax_key$Level3 <- base::sub('_X', '', tax_key$Level3)
tax_key$Level3 <- base::sub('__', 'Other', tax_key$Level3)
...
index_frame %>%
  ...
  xlab('Number Clusters')+
  theme_bw()
silhouette_profile_kmed8<-cluster::silhouette(kmed_clusts[[4]])
...
ggplot(new_silframe,aes(x=idno,y=dist))+
  ...
  facet_wrap(~cluster)
medoid_otus<-kmed_clusts[[4]]$medoids
